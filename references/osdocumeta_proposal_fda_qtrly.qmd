...
-   3.1 Core Variables (come from the define.xml specificaions) -4.2 Data Dependencies (may need to some code scraping to read through each ADaM program and identify any other ADaMs being sourced in to create the ADaM) -5.2 Analysis Datasets (read in all of the .xpts in the submission package to extract the metadata for each ADaM to fill in this table) -6.2 Issues Summary (extract report from P21) -7.2 ADaM Programs (read in all of the .xpts in the submission package to extract the list of ADaMs and it’s label for each ADaM to fill in this table) -7.3 Analysis Output Programs (this one may be tricky as there are many study specific Selection Criteria to include) -7.4 Proprietary R Packages (Joel has some code for this, in R, essentially using SessionInfo() and {renv}) eCTD structure.
